...
Now `model` is functionally equivalent to `Net()`, and all `torch` functionalities are still available, making `model` is ready for co-design configuration and/or optimization, at training time or post-training. 
