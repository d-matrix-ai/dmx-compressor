...
def test_castto_bfp16_1():
    ...
    _x = numerical.CastTo(format="BFP[8|8]{1}(SN)")(x)
...
def test_bfp16_1_rounding():
    ...
    assert torch.all(numerical.CastTo(format="BFP[8|8]{1}(SN)")(x) == y)
    assert torch.all(numerical.CastTo(format="BFP[8|8]{1}(SN)")(-x) == -y)
...
def test_bfp12_1_rounding():
    ...
    assert torch.all(numerical.CastTo(format="BFP[4|8]{1}(SN)")(x) == y)
    assert torch.all(numerical.CastTo(format="BFP[4|8]{1}(SN)")(-x) == -y)
