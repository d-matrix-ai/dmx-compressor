...
from benchmark import measure_model_runtime,measure_model_accuracy,measure_model_error,EVALUATION_MODE
...
def clip_evaluator(model,device,desc):
    ...
    ds= datasets.load_dataset('HuggingFaceM4/COCO',split = 'test',
    ...
    for indices in tqdm(dl):
        ...
        model_input = processor(text=[x['raw'] for x in batch['sentences']],
    ...
    correct_winner_mask = (ordered_winners == \
                           torch.arange(ordered_winners.shape[0]).unsqueeze(1).to(ordered_winners.device)).long()
    top_K = [1,5,10]
    metrics = {f'top_{k}' : correct_winner_mask[:,:k].sum(-1).float().mean() for k in top_K}
    return metrics
...
def clip_model_maker(return_kwargs = False):
    ...
    inputs = processor(text=["a photo of a cat", "a photo of a dog"], images = ds[:2]['image'],
                       return_tensors="pt", padding=True)

    model_forward_kwargs = {'input_ids' : inputs['input_ids'].to(device),
...
def clip_text_model_maker():
    model,_,clip_evaluator,device,model_forward_kwargs = clip_model_maker(return_kwargs = True)
...
def main():
    active_modes = [EVALUATION_MODE.VANILLA,EVALUATION_MODE.BASELINE,
                    EVALUATION_MODE.BASIC,EVALUATION_MODE.BASIC_NOVSIMD]
