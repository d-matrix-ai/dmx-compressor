...
def prepare_model(model,evaluation_mode,model_runner):
    ...
    if evaluation_mode == EVALUATION_MODE.BASELINE:
        ...
        model.to_basic_mode()
        model_runner(model)
        vsimd_modules = [(x[0],x[1]) for x in model.named_dmx_modules() \
...
def measure_mode_perf(model : torch.nn.Module,
                      model_runner,
                      device : torch.device,
    ...
    model,vsimd_modules_by_type = prepare_model(model,evaluation_mode,
                                                model_runner)
    ...
    if evaluation_mode == EVALUATION_MODE.VANILLA:
        ...
        return {'total_time' : (t2 - t1)/n_measure_runs,
                'per_layer_times' : {}}
    else:
        ...
        average_perlayer_runtimes = {k:sum([sum(single_run_runtime[k]) \
                                            for single_run_runtime in all_runtimes])/len(all_runtimes) for \
                                     k in mod_names}
...
def evaluate_vsimd_ops_deltas(results : Dict[str,Any]):
    ...
    if b_val in results and b_novsimd_val in results:
        ...
        for type_name,layer_names in vsimd_modules_by_type.items():
            time_basic = sum(results[b_val]['per_layer_times'][lay_name] for lay_name in layer_names)
            time_basic_novsimd = sum(results[b_novsimd_val]['per_layer_times'][lay_name] for lay_name in layer_names)            
            vsimd_time_deltas[type_name] = time_basic - time_basic_novsimd 

        return vsimd_time_deltas

    print('Specify EVALUATION_MODE.BASIC and EVALUATION_MODE.BASIC_NOVSIMD  \
    to obtain vsimd op deltas',list(results.keys()))
    return None

def measure_model_runtime(model_maker : Callable[[],Tuple[torch.nn.Module,
                                                          List[Any],Dict[str,Any],Callable,torch.device]],
    ...
    main_table_str = tabulate({' ':['max memory (GB)','total time (s)',*layer_names],
                          **{k : [results[k]['max_memory'] / 2**30,
                                  results[k]['total_time'],
                                  *results[k]['per_layer_times'].values()] for k in results}
    ...
    if vsimd_time_deltas is not None:
        ...
        cumulative_runtime = (torch.cumsum(torch.Tensor([0] + lay_deltas),dim = 0) + \
                                          results[EVALUATION_MODE.BASIC_NOVSIMD.value]['total_time']).numpy().tolist()
                                           
        vsimd_deltas_table_str = tabulate({'Layer type' : ['(BASIC mode time without VSIMD ops)'] + lay_types,
                                           'Time deltas' : [' '] + lay_deltas,
                                           'Total run time' : cumulative_runtime},headers = 'keys',tablefmt = 'github')
...
def measure_model_accuracy(model_maker : Callable[[],Tuple[torch.nn.Module,
                                                          List[Any],Dict[str,Any],Callable,torch.device]],
                          modes : List[EVALUATION_MODE]):
    results = {}
    for mode in modes:
        ...
        model, _ = prepare_model(model,mode,
                                 model_runner)
        
        results[mode.value] = evaluation_fn(model,device,mode.value)    

    metric_names = list(list(results.values())[0].keys())
    main_table_str = tabulate({' ':metric_names,
                          **{k : [results[k][metric] for metric in metric_names]
                             for k in results}
                          },headers = 'keys',tablefmt = 'github')
    print(main_table_str)
        
def collect_layer_activations(model_maker,mode):
    model,model_runner,evaluation_fn,device = model_maker()
    model, _ = prepare_model(model,mode,
                             model_runner)
    ...
    if mode == EVALUATION_MODE.VANILLA:
        ...
        mods_dict = {'.'.join(x[0].split('.')[1:]): x[1] for x in model.named_dmx_modules()}
        with torch.no_grad():
...
def compute_mse_error(out1,out2) -> float:
    if isinstance(out1,torch.Tensor):
        return ((out1.float() - out2.float())**2).float().mean().item()
    elif isinstance(out1,(tuple,list)):
        return sum(compute_mse_error(x,y) for x,y in zip(out1,out2))
    elif isinstance(out1,dict):
        return sum(compute_mse_error(out1[k],out2[k]) for k in out1.keys())
    else:
        return 0

def compute_maxdelta_error(out1,out2) -> float:
    if isinstance(out1,torch.Tensor):
        return ((out1.float() - out2.float())**2).float().abs().max().item()
    elif isinstance(out1,(tuple,list)):
        return max([compute_maxdelta_error(x,y) for x,y in zip(out1,out2)] + [0])
    elif isinstance(out1,dict):
        return max([compute_maxdelta_error(out1[k],out2[k]) for k in out1.keys()] + [0])
    else:
        return 0

def compute_error(out1,out2):
    return {'mse' : compute_mse_error(out1,out2),
            'maxdelta' : compute_maxdelta_error(out1,out2)}
    

def measure_model_error(model_maker : Callable[[],Tuple[torch.nn.Module,
                                                          List[Any],Dict[str,Any],Callable,torch.device]],
                          modes : List[EVALUATION_MODE],
                          reference_mode  : EVALUATION_MODE= EVALUATION_MODE.BASELINE ):

    print(f'collecting activations for reference {reference_mode.value}')
    _,reference_activations,reference_final_output = collect_layer_activations(model_maker,reference_mode)
    ...
    for mode in modes:
        ...
        mods_dict,activations,final_output = collect_layer_activations(model_maker,mode)
        if activations and reference_activations:
            ...
            for l_name in lay_names:
                ...
                for datum1,datum2 in zip(reference_activations[l_name],
                                         activations[l_name]):
                    for metric,val in compute_error(datum1['input'],datum2['input']).items():
                    ...
                    model2_clean_input_output = mods_dict[l_name](*input_args,**input_kwargs)

                    for metric,val in compute_error(out1,model2_clean_input_output).items():
                        per_layer_error_dict[l_name][metric] += val
                    
            results[mode.value] = {'cumulative' : cum_error_dict,
                                   'per_layer' : per_layer_error_dict,
                                   'input' : input_error_dict}
            if len(lay_names) > len(table_lay_names):
                table_lay_names = lay_names
        
        results[mode.value]['final_output_error']  = compute_error(final_output,
    ...
    for k in results:
        for e_type in error_types:
            if e_type in results[k]:
                table_dict[f'{k}({e_type})'] = [error_format,make_error_str(results[k]['final_output_error'])] + \
                    [make_error_str(results[k][e_type][l_name]) for l_name in table_lay_names]
        if not any(e_type in results[k] for e_type in error_types):
            table_dict[k] = [error_format,make_error_str(results[k]['final_output_error'])] 
    table_str = tabulate({f'error relative to {reference_mode.value}' : ['error_format','final_output_error'] + table_lay_names,
                          **table_dict},headers = 'keys',tablefmt = 'github')
