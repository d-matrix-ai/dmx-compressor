...
def prune_graph(
        ...
        for node in graph_dict:
            if debug:
                print(
                    node,
                    graph_dict[node]["op"],
                    [i for i in graph_dict[node]["input_name"]],
                )
    ...
    for node in graph_dict:
        ...
        if (
            graph_dict[node]["op"] in ops_to_prune
            or any(
                [
                    name in graph_dict[node]["op"]
                    for name in [
                        "layer_norm",
                        "embedding",
                        "cast",
            ...
            or "add" in node
            and graph_dict[node]["input_name"][1][0] == "("
        ) and graph_dict[  # second input is a tuple
            node
        ][
            "input_name"
        ] != []:
