import torch
from ..number import Number, FixedPoint, BlockFloatingPoint, FloatingPoint
...
def block_quantize(x, wl, dim=-1, rounding="stochastic"):
    ...
    assert rounding in ["stochastic", "nearest","down","up"], "invalid rounding mode, {}".format(rounding)
    quant_module = get_module(x)
    if rounding == "nearest":
