...
Tensor block_quantize_down_cuda(Tensor a, int wl, int dim) {
  ...
  block_kernel_down<<<blockNums, blockSize>>>(a.data_ptr<float>(),
                                                 o.data_ptr<float>(),
                                                 size,
                                                 max_entry.data_ptr<float>(),
                                                 wl);
...
Tensor block_quantize_up_cuda(Tensor a, int wl, int dim) {
  ...
  block_kernel_up<<<blockNums, blockSize>>>(a.data_ptr<float>(),
                                                 o.data_ptr<float>(),
                                                 size,
                                                 max_entry.data_ptr<float>(),
                                                 wl);
...
Tensor float_quantize_stochastic_cuda(Tensor a, int man_bits, int exp_bits, int exp_bias) {
...
Tensor float_quantize_nearest_cuda(Tensor a, int man_bits, int exp_bits, int exp_bias) {
  // use external random number right now
  auto o = zeros_like(a);
  int size = a.numel();
  int blockSize = 1024;
