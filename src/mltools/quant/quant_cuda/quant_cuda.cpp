...
{
  CHECK_INPUT(a);
  return float_quantize_nearest_cuda(a, man_bits, exp_bits, exp_bias);
}
...
{
  CHECK_INPUT(a);
  return float_quantize_stochastic_cuda(a, man_bits, exp_bits, exp_bias);
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m)
{
  ...
  m.def("block_quantize_down", &block_quantize_down, "Block Floating Point Number Roundinng Down Quantization (CUDA)");
