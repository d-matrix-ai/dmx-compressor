...
def fixed_point_quantize(x, wl, fl, clamp=True, symmetric=False, rounding="stochastic"):
    ...
    assert rounding in ["stochastic", "nearest", "up", "down"]
    assert_wl_fl(wl, fl)
    ...
    if rounding == "nearest":
        ...
        out = quant_module.fixed_point_quantize_up(
            x.contiguous(), wl, fl, clamp, symmetric
        )
    elif rounding == "down":
        out = quant_module.fixed_point_quantize_down(
            x.contiguous(), wl, fl, clamp, symmetric
        )
    return out
...
def float_quantize(x, exp, man, bias=None, flush_subnormal=True, rounding="stochastic"):
    ...
    if rounding == "nearest":
        out = quant_module.float_quantize_nearest(
            x.contiguous(), man, exp, bias, flush_subnormal
        )
    elif rounding == "stochastic":
