...
Tensor fixed_point_quantize_up(Tensor a, int wl, int fl, bool clamp, bool symmetric)
{
  CHECK_INPUT(a);
  auto a_array = a.data_ptr<float>();
  Tensor o = zeros_like(a);
  ...
  for (int64_t i = 0; i < size; i++)
  {
    o_array[i] = up_round(a_array[i], sigma);
    if (clamp)
    {
      o_array[i] = clamp_helper(o_array[i], t_min, t_max);
    }
...
Tensor fixed_point_quantize_down(Tensor a, int wl, int fl, bool clamp, bool symmetric)
{
  CHECK_INPUT(a);
  auto a_array = a.data_ptr<float>();
  Tensor o = zeros_like(a);
  ...
  for (int64_t i = 0; i < size; i++)
  {
    o_array[i] = down_round(a_array[i], sigma);
    if (clamp)
    {
      o_array[i] = clamp_helper(o_array[i], t_min, t_max);
    }
...
unsigned int round_bitwise(unsigned int target, int man_bits, Mode rounding)
{
  ...
  else if (rounding == rNearest)
  {
    rand_prob = 1 << (23 - man_bits - 1);
    if (target << (9 + man_bits) >> (9 + man_bits) == (unsigned int)(1 << 22 - man_bits))
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m)
{
  ...
  m.def("fixed_point_quantize_up", &fixed_point_quantize_up, "Fixed Point Number Rounding Up Quantization (CPU)");
  m.def("fixed_point_quantize_down", &fixed_point_quantize_down, "Fixed Point Number Rounding Down Quantization (CPU)");
