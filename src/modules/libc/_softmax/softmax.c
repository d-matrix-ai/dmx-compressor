...
linlut_cfg inv_linlut_cfg;
  ...
  printf(" \n FINAL COMPARISON \n");
  for(i=0; i<8; i++) if (i<16) printf(" ideal8bsmax[%0d] = %d,  dmtrx8bsmax[%0d] = %d \n", i, ideal8bsmax[i], i, dmtrx8bsmax[i]);
...
static int    dmtrx8bsmax[vecblk_len];
  ...
  idx_gen(input, vecblk_len, cfg, ecfg, idx_mu); 
  intrp_lut(idx_mu, vecblk_len, 1, ExpLUT, exp_vec);

  for(i=0; i<vecblk_len; i++) sumexp += exp_vec[i];
