...
qconfig.
    ...
    * kernel_scale) / (output_scale)`. Using this eps (w/ assumed max value
    ...
    threshold.
"""
default_symmetric_qnnpack_qconfig = QConfig(activation=HistogramObserver.with_args(dtype=Format,
                                                                                   reduce_range=False,
                                                                                   eps=2 ** -12),
...
_ObserverConstructor = Union[_PartialWrapper, Type[DMXObserverBase]]

def _obs_or_fq_ctr_equals(obs_or_fq1: _ObserverConstructor, obs_or_fq2: _ObserverConstructor):
...
def _activation_is_memoryless(qconfig: QConfig):
    ...
    if isinstance(act) and hasattr(act, "activation_post_process"):
