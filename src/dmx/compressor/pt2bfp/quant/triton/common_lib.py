...
'''
Code derived from https://github.com/microsoft/microxcaling/blob/b601f4e946fffd702ec8f2d41980b1788557d1b1/mx/tests/common_lib.py#L31
'''
...
'''
Code derived from https://github.com/microsoft/microxcaling/blob/b601f4e946fffd702ec8f2d41980b1788557d1b1/mx/tests/common_lib.py#L42
'''
def check_diff(y1, y2, tol=0, ntol=0, handle_infs=False):
    ...
    ndiff = np.sum(diff > tol)     
    
    plt.imshow(diff[:64, :64], cmap='hot', interpolation='nearest', cbar=True,)
...
'''
Code derived from https://github.com/microsoft/microxcaling/blob/b601f4e946fffd702ec8f2d41980b1788557d1b1/mx/tests/common_lib.py#L91
'''
def check_diff_quantize(x, y1, y2, tol=0, ntol=0, handle_infs=False):
    ...
    #     print("y1 and y2 exp match:  ", check_exp_match(y1[where_diff][0], y2[where_diff][0]))
    #     if (np.count_nonzero(where_diff) > 1):
    #         print("y1 and y2 exp match:  ", check_exp_match(y1[where_diff][-1], y2[where_diff][-1]))
    #     raise ValueError
    ...
    if (max_diff > tol and ndiff > ntol) or np.any(torch_infs != cuda_infs):
        ...
        print("y1 and y2 exp match:  ", check_exp_match(y1[where_diff][0], y2[where_diff][0]))
        print("y1 and y2 mantissa match:  ", check_man_match(y1[where_diff][0], y2[where_diff][0]))
        if (np.count_nonzero(where_diff) > 1):
            print("--------------------")
...
# floating-point
# Code derived from https://github.com/microsoft/microxcaling/blob/b601f4e946fffd702ec8f2d41980b1788557d1b1/mx/tests/common_lib.py#L142
#--------------------------------------------------------------------
_CACHE = {}
