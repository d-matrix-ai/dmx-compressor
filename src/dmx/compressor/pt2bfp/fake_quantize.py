...
class FakeQuantizeBase(ABC, Module):
    ...
    def __init__(self):
        ...
        # self.register_buffer('fake_quant_enabled', torch.tensor([1], dtype=Format))
        # self.register_buffer('observer_enabled', torch.tensor([1], dtype=Format))
    ...
    * :attr:`dtype` specifies the quantized dtype that is being emulated with fake-quantization,
    ...
    def __init__(self, observer=MovingAverageMinMaxObserver, quant_min=None, quant_max=None, is_dynamic=False, **observer_kwargs):
        ...
        if quant_min is not None and quant_max is not None:
            ...
            dtype = observer_kwargs.get("dtype", Format)
...
default_fake_quant = FakeQuantize.with_args(observer=MovingAverageMinMaxObserver, quant_min=0, quant_max=255,
                                            dtype=Format, qscheme=torch.per_tensor_affine, reduce_range=True)
"""
...
default_weight_fake_quant = FakeQuantize.with_args(observer=MovingAverageMinMaxObserver, quant_min=-128, quant_max=127,
                                                   dtype=Format, qscheme=torch.per_tensor_symmetric, reduce_range=False)
"""
...
default_dynamic_fake_quant = FakeQuantize.with_args(
    ...
    dtype=Format, averaging_constant=1)
"""
...
def _is_fake_quant_script_module(mod):
    ...
    if isinstance(mod, torch.jit.RecursiveScriptModule):
        ...
        return name == 'pt2bfp.fake_quantize.FakeQuantize' or \
            name == 'pt2bfp.fake_quantize.FusedMovingAvgObsFakeQuantize'
    return False
