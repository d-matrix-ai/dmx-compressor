...
# from _PartialWrapper import with_args

from dmx.compressor.pt2bfp.fake_quantize import(
    FixedQParamsFakeQuantize)

from dmx.compressor.pt2bfp import _DerivedObserverOrFakeQuantize
...
from dmx.compressor.pt2bfp.quantize import (
    convert
)
...
from dmx.compressor.pt2bfp.quantizer import (
    ...
    FixedBFPQParamsQuantizationSpec,
    SharedBFPQuantizationSpec,
    DerivedBFPQuantizationSpec,
)
from dmx.compressor.pt2bfp import ObserverOrFakeQuantize
...
def _get_observer_kwargs(quant_spec: Union[BFPQuantizationSpec, FixedBFPQParamsQuantizationSpec]):
...
def _create_obs_or_fq_from_qspec(
    ...
    if quantization_spec is None:
        return None
    if isinstance(quantization_spec, SharedBFPQuantizationSpec):
    ...
    elif isinstance(quantization_spec, DerivedBFPQuantizationSpec):
    ...
    elif isinstance(quantization_spec, FixedBFPQParamsQuantizationSpec):
    ...
    assert isinstance(quantization_spec, BFPQuantizationSpec)
    ...
    # need to check what this is - fix bug
    # added __class__ to access __name__
    if "PerChannel" not in obs_or_fq_class.__class__.__name__:  # type: ignore[operator, union-attr]
        kwargs.pop("ch_axis")
                    ...
                    if (
                        ...
                        for user in node.args[0].users:
                            ...
                            is_user_quantized = (
                                ...
                                (user.op == 'call_module' and isinstance(named_modules[str(user.target)], DMXObserverBase))
                            )
