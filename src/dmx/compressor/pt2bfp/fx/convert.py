...
from ..observer import _is_activation_post_process
from torch.ao.quantization.fx.graph_module import (
    _is_observed_module,
    _is_observed_standalone_module,
)
from torch.ao.quantization.fx._equalize import update_obs_for_equalization, convert_eq_obs
...
from ._decomposed import quant_dec_lib
# from torch.ao.quantization.fx._decomposed import quantized_decomposed_lib  # noqa: F401

_QSCHEME_TO_CHOOSE_QPARAMS_OP = {
    torch.per_tensor_affine: torch.ops.quant_decomposed.choose_qparams.tensor, # pt2bfp.fx.ops.
    torch.per_tensor_symmetric: torch.ops.quant_decomposed.choose_qparams_symmetric.tensor,
}
# _QSCHEME_TO_CHOOSE_QPARAMS_OP = {
...
def _replace_observer_with_quantize_dequantize_node_decomposed(
    ...
    if dtype in [torch.quint8, torch.qint8, torch.qint32, torch.uint8, torch.int8, torch.int16, torch.int32] and \
        ...
        dtype_ = to_underlying_dtype(dtype)
        assert dtype_ in [torch.uint8, torch.int8] or isinstance(dtype, BlockFloatingPoint), \
        ...
        if is_per_channel(activation_post_process.qscheme):  # type: ignore[attr-defined]
            ch_axis = int(activation_post_process.ch_axis)  # type: ignore[attr-defined, arg-type]
            quantize_op = torch.ops.quant_decomposed.quantize_per_channel.default
            dequantize_op = torch.ops.quant_decomposed.dequantize_per_channel.default
            quant_min = activation_post_process.quant_min
            quant_max = activation_post_process.quant_max
            dtype_ = to_underlying_dtype(dtype)
            qparams = {
            ...
            quantize_op = torch.ops.quant_decomposed.quantize_per_tensor.default
            dequantize_op = torch.ops.quant_decomposed.dequantize_per_tensor.default
            scale = float(scale)
            zero_point = int(zero_point)
            quant_min = activation_post_process.quant_min  # type: ignore[attr-defined]
            quant_max = activation_post_process.quant_max  # type: ignore[attr-defined]
