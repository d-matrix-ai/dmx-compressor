...
from dmx.compressor.pt2bfp.utils import check_min_max_valid, determine_qparams
...
quant_dec_lib = Library("quant_decomposed", "DEF")

_INTEGER_DTYPES = [torch.uint8, torch.int8, torch.int16, torch.int32]
_FLOAT_DTYPES = [torch.float8_e5m2, torch.float8_e4m3fn, BlockFloatingPoint, MXFP]
...
quant_dec_lib.define(
    "quantize_per_tensor(Tensor input, float scale, int zero_point, "
    "int quant_min, int quant_max, ScalarType dtype) -> Tensor")

@impl(quant_dec_lib, "quantize_per_tensor", "CompositeExplicitAutograd")
def quantize_per_tensor(
    ...
    inv_scale = 1.0 / scale
    if isinstance(dtype, BlockFloatingPoint):
        elem_format = dtype
        rounding = elem_format.rounding
        symmetric = elem_format.symmetric

        _x_triton = _quantize_bfp(input, 16, elem_format,
                        axes=[-1],
                        block_size=1024,
                        round=rounding,
                        symmetric=symmetric,
...
@impl(quant_dec_lib, "quantize_per_tensor", "Meta")
def quantize_per_tensor_meta(
        ...
        quant_min: int,
        quant_max: int,
        dtype: Union[torch.dtype, Format]
) -> torch.Tensor:
    if input.dtype in [torch.float16, torch.bfloat16]:
        input = input.to(torch.float32)
    assert input.dtype == torch.float32, f"Expecting input to have dtype torch.float32, but got dtype: {input.dtype}"
...
@impl(quant_dec_lib, "quantize_per_tensor.tensor", "CompositeExplicitAutograd")
...
@impl(quant_dec_lib, "quantize_per_tensor.tensor", "Meta")
...
@impl(quant_dec_lib, "quantize_per_tensor.tensor2", "CompositeExplicitAutograd")
...
@impl(quant_dec_lib, "quantize_per_tensor.tensor2", "Meta")
...
@impl(quant_dec_lib, "dequantize_per_tensor", "CompositeExplicitAutograd")
...
@impl(quant_dec_lib, "dequantize_per_tensor", "Meta")
...
@impl(quant_dec_lib, "dequantize_per_tensor.tensor", "CompositeExplicitAutograd")
...
@impl(quant_dec_lib, "dequantize_per_tensor.tensor", "Meta")
...
@impl(quant_dec_lib, "dequantize_per_tensor.tensor2", "CompositeExplicitAutograd")
...
@impl(quant_dec_lib, "dequantize_per_tensor.tensor2", "Meta")
...
@impl(quant_dec_lib, "choose_qparams.tensor", "CompositeExplicitAutograd")
def choose_qparams_tensor(
    ...
    assert input.dtype in [
        ...
        torch.float16,
        torch.bfloat16,
    ] or isinstance(input.dtype, BlockFloatingPoint), f"Expecting input to have dtype torch.float32/16/b16, but got dtype: {input.dtype}"
...
@impl(quant_dec_lib, "choose_qparams_symmetric.tensor", "CompositeExplicitAutograd")
...
@impl(quant_dec_lib, "choose_qparams.tensor", "Meta")
...
@impl(quant_dec_lib, "choose_qparams_symmetric.tensor", "Meta")
...
@impl(quant_dec_lib, "quantize_per_channel", "CompositeExplicitAutograd")
def quantize_per_channel(
        ...
        quant_min: int,
        quant_max: int,
        dtype: Union[torch.dtype, BlockFloatingPoint]
) -> torch.Tensor:
...
@impl(quant_dec_lib, "quantize_per_channel", "Meta")
...
@impl(quant_dec_lib, "dequantize_per_channel", "CompositeExplicitAutograd")
...
@impl(quant_dec_lib, "dequantize_per_channel", "Meta")
...
@impl(quant_dec_lib, "quantize_per_token", "CompositeExplicitAutograd")
...
@impl(quant_dec_lib, "quantize_per_token", "Meta")
...
@impl(quant_dec_lib, "dequantize_per_token", "CompositeExplicitAutograd")
...
@impl(quant_dec_lib, "dequantize_per_token", "Meta")
...
@impl(
    quant_dec_lib, "quantize_per_channel_group", "CompositeExplicitAutograd"
)
...
@impl(quant_dec_lib, "quantize_per_channel_group", "Meta")
...
@impl(quant_dec_lib, "fake_quant_per_channel", "Autograd")
...
@impl(quant_dec_lib, "fake_quant_per_channel", "Meta")
