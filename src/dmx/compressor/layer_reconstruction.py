    ...
    def slanc_tuning(self, hyperparams) -> None:
        if (
            ...
            if hyperparams.position == "post_attn":
                ...
                prev_ln_weight = (
                    hyperparams.prev_ln_weight.get_parameter("weight")
                    .detach()
                    .to(torch.float32)
                )
                W_V = (
                ...
                A = hyperparams.fc1.get_parameter("weight").detach().to(torch.float32)
                B = hyperparams.fc2.get_parameter("weight").detach().to(torch.float32)
                norm = (
            ...
            elif hyperparams.position == "post_mlp" and hyperparams.mlp_type == "llama":
                prev_ln_weight = (
                    hyperparams.prev_ln_weight.get_parameter("weight")
                    .detach()
                    .to(torch.float32)
                ...
                norm = torch.linalg.norm(
                    W_down @ (W_up * prev_ln_weight), "fro"
...
class OptimalBrainCompressor:
    ...
    def apply(self, microblock_size=1, block_size=128, percdamp=0.01):
        ...
        H = self.H.to(self.module.weight.data.device)
        del self.H
        dead = torch.diag(H) == 0
        H[dead, dead] = 1
        W[:, dead] = 0
